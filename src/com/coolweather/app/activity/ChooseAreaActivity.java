...
public class ChooseAreaActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/**
		 * 只有city_selected为true和不是从WeatherActivity中跳转过来，才会返回到WeatherActivity中，
		 * 其它情况就悬着城市
		 */
		/*
		 * if(prefs.getBoolean("city_selected", false) &&
		 * !isFromWeatherActivity){ Intent intent = new
		 * Intent(this,WeatherActivity.class); startActivity(intent); finish();
		 * return; }
		 */
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (currentLevel == LEVEL_PROVINCE) {
					...
					queryCities();
					LogUtil.i("coolweather", "*******selectProvince: "
							+ selectProvince.getProvincePyName());
				} else if (currentLevel == LEVEL_CITY) {
					selectCity = cityList.get(position);
					queryCounties();
				} else if (currentLevel == LEVEL_COUNTY) {
					String countyWeatherCode = countyList.get(position)
							.getCountyUrl() + "";
					Intent intent = new Intent(ChooseAreaActivity.this,
							WeatherNewActivity.class);
		...
		if (provinceList.size() > 0) {
			...
			for (Province province : provinceList) {
				...
				LogUtil.i("coolweather",
						"for循环： " + province.getProvincePyName());
			}
	...
	private void queryCities() {
		...
		LogUtil.i("coolweather",
				"queryCities() " + selectProvince.getProvincePyName());
		if (cityList.size() > 0) {
			dataList.clear();
	...
	public void onBackPressed() {
		if (currentLevel == LEVEL_COUNTY) {
			...
			Intent intent = new Intent(this, WeatherNewActivity.class);
			startActivity(intent);
